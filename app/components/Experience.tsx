...
interface Treatment {
  ...
  desc: string;
  duration: string;
  images: string[]; // Now multiple images
  perks?: string[]; // Optional perks
}
...
const treatments: Treatment[] = [
  {
    ...
    duration: "90 min",
    images: ["/images/exp3.png", "/images/spa3.png", "/images/spa4.png"],
    perks: ["Calming environment", "Essential oils", "Expert therapists"],
  },
  {
    title: "Hydra-Radiance Facial",
    desc: "Advanced hydration therapy that leaves skin glowing and youthful.",
    duration: "60 min",
    images: ["/images/exp4.png", "/images/spa1.png", "/images/spa2.png"],
    perks: ["Hydrating serum", "LED therapy", "Relaxing ambiance"],
  },
  {
    title: "Hot Stone Ritual",
    desc: "Warmed volcanic stones combined with expert touch for profound tension release.",
    duration: "120 min",
    images: ["/images/exp2.png", "/images/spa5.png", "/images/spa6.png"],
    perks: ["Hot stones", "Deep massage", "Stress relief"],
  },
  {
    title: "Thermal Spring Bath",
    desc: "Mineral-rich waters sourced from local springs for natural detoxification.",
    duration: "Unlimited",
    images: ["/images/exp1.png", "/images/spa7.png", "/images/spa8.png"],
    perks: ["Mineral-rich water", "Detox therapy", "Relaxing atmosphere"],
  },
];
  ...
  return (
    <section id="spa" className="py-24 bg-[#011f15] relative">
      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
        {/* Main Spa Section content unchanged */}
              ...
              <div className="flex -space-x-3">
                {[1, 2, 3, 4].map((i) => (
                  <Image
                    ...
                    width={48}
                    height={48}
                    className="rounded-full border-2 border-[#011f15] shadow-lg"
                    alt="Reviewer"
                  />
                ))}
              </div>
        ...
        {/* Treatments grid unchanged */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {treatments.map((t, idx) => (
            <div
              key={idx}
              className="bg-surface-dark/40 backdrop-blur-lg border border-white/5 rounded-4xl overflow-hidden hover:border-primary/30 transition-all duration-500 group"
            >
              ...
              <div className="p-8 space-y-4">
                ...
                <p className="text-gray-400 text-sm font-light leading-relaxed">{t.desc}</p>

                {/* Discover More button */}
                <button
                  onClick={() => handleOpenDialog(t)}
                  className="inline-flex gap-6 items-center text-xs font-bold uppercase tracking-widest text-[#C89F65] hover:gap-2 transition-all"
                >
      ...
      {/* Dialog modal for selected treatment */}
      {selectedTreatment && (
        <Dialog open={isDialogOpen} onOpenChange={handleCloseDialog}>
          <DialogContent className="max-w-3xl w-full rounded-xl p-6">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold mb-4">{selectedTreatment.title}</DialogTitle>
            </DialogHeader>

            {/* Scrollable horizontal images */}
            <div className="flex overflow-x-auto gap-4 mb-4 pb-2 no-scrollbar">
              {selectedTreatment.images.map((img, i) => (
                <Image
                  key={i}
                  src={img}
                  alt={`${selectedTreatment.title} ${i}`}
                  width={400}
                  height={250}
            ...
            <p className="mb-4 text-gray-600">{selectedTreatment.desc}</p>

            {/* Perks */}
            {selectedTreatment.perks && (
              <ul className="mb-4 list-disc list-inside space-y-1 text-gray-600">
                {selectedTreatment.perks.map((perk, i) => (
                  <li key={i}>{perk}</li>
                ))}
              </ul>
            )}
            ...
            <div className="flex justify-between items-center mt-6">
              <p className="font-semibold text-lg text-white">{selectedTreatment.duration}</p>
              <button
